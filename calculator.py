    ...
    if txtInput == "help":
      ...
      if (len(numlist) % 2) != (len(operlist) % 2):
        ...
        for op in operlist:
          ...
          for ind in sorted(poplist, reverse=True):
            bracklist.pop(ind)
          operation = f'{operation} {op} {opbra}{numlist[i]}'
          i+=1
          opbra = ''
        if bracklist:
...
def normal_number(number, date_format):
  ...
  if re.search(f"(\d({'|'.join(unitDateConverter)}))",number):
    date_format = True
    for date in date_re :
    ...
    if number != "":
      print(f'Error unknow date format, left : {number} after filtering known values')
    number = final
  else:
    decalage = number.find(".")
    if decalage == -1:
      pass # number = todec(number, 10)
    else:
      # - - - - - Parti à revoir pour les cas de dicNumber décimal
      number = number.replace(".","")
      decalage = len(number) - decalage
      number = str(number) # number = str(todec(number, 10))
      number = f"{number[:len(number) - decalage]}.{number[len(number) - decalage:]}"
  return [number, date_format]

def todec(innumber, base):
  global dicNumber
  innumber, frac = innumber.split('.') if '.' in innumber else [innumber, '']
  unit = value = 0
  valfrac = float(0)
  if len(innumber) != 0:
    if innumber[-1] != "]":
  ...
  if len(frac) != 0:
    ...
    if len(frac) != 0:
      valfrac += todec(f".{frac}", base) / base
    value += valfrac
  return value
...
def tobase (innumber, base):
  ...
  innumber, frac = innumber.split('.') if '.' in innumber != -1 else [innumber, 0]
  innumber = int(innumber)
  frac = float(f'0.{frac}')
  while innumber != 0:
    scale = int(innumber % base)
    if scale < len(dicNumber) :
      number = f'{dicNumber[scale]}{number}'
    else :
      number = f'[{scale}]{number}'
    innumber = (innumber - scale) / base
  if frac > 0:
    number+= '.'
    global nbdecimal
    decibase = base
    for _ in range(nbdecimal):
      scale = int(frac / (1 / decibase))
      frac = frac - (scale * (1 / decibase))
      number += dicNumber[scale] if len(dicNumber) > scale else f'[{scale}]'
      if frac == 0:
        break
      decibase = decibase * base
  return ('-' if neg else '' ) + number # .decode('latin_1')
  ...
  if hasattr(newDicNumber, 'append'):
    ...
    with open(configFile, 'r') as cf:
      ...
      if newDicNumber in config['dicNumber']:
        ...
        print(f"Error unknown {newDicNumber} standart format")
        return False
  else :
...
def flathour(hfrom) :
  ...
  print(f'diff = {hdif[2]}:{hdif[1]}:{hdif[0]}')
